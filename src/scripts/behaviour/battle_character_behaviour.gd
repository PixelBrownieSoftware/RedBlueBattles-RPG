...
func check_cond(character: battle_character_data, target: battle_character_data, skill: rpg_skill, turn_number: int, round_number: int) -> bool:
	...
	match subject:
		...
		SUBJECT.EVERY_X_ROUND:
			var fix = round_number
			if fix == 0:
				fix = 1
			result = (fix % exact_number == 0) #if exact_number != 0 else false
			print("Current Round: " + str(round_number) + " Is correct round: "+ str(result) + " out of round: " + str(exact_number))	
